...
import com.epiroc.workflow.common.system.constant.SymbolConstant;
...
public class oConvertUtils {
	...
	/**
	 * 将一个 Entity 转换为 Model：
	 * 1. 先使用 BeanUtils.copyProperties 复制名称相同的字段。
	 * 2. 再用自定义的字段映射表，手动复制那些名称不相同的字段。
	 *
	 * @param entity       源对象
	 * @param modelClass   目标对象类型
	 * @param fieldMapping 自定义字段映射：key 为 entity 中的属性名，value 为 model 中的属性名
	 * @param <F>          源类型
	 * @param <T>          目标类型
	 * @return 转换后的 model 实例
	 * 用法示例：
	 * Map<String, String> mapping = new HashMap<>();
	 * mapping.put("operatorEmail", "updateBy");
	 * mapping.put("orderId", "id");
	 * Target target = oConvertUtils.entityToModel(source, target.class, mapping);
	 */
	public static <F, T> T entityToModel(F entity,
										 Class<T> modelClass,
		...
		try {
			...
			log.error("entityToModel: 无法实例化 " + modelClass.getName(), e);
			return null;
		}

		// 1. 先复制所有同名属性，但忽略那些在自定义映射表里有特别处理的源字段
		String[] ignoreProps = fieldMapping == null
				? new String[0]
				: fieldMapping.keySet().toArray(new String[0]);
		BeanUtils.copyProperties(entity, model, ignoreProps);

		if (fieldMapping != null && !fieldMapping.isEmpty()) {
			// 2. 手动复制需要映射的属性
			BeanWrapper srcWrap = new BeanWrapperImpl(entity);
			BeanWrapper tgtWrap = new BeanWrapperImpl(model);

			fieldMapping.forEach((srcProp, tgtProp) -> {
				try {
					Object val = srcWrap.getPropertyValue(srcProp);
					tgtWrap.setPropertyValue(tgtProp, val);
				} catch (Exception e) {
					log.warn(String.format(
							"entityToModel: 字段映射失败 [%s -> %s]，值：%s",
							srcProp, tgtProp,
							srcWrap.getPropertyValue(srcProp)), e);
				}
			});
		}
	 ...
	 * @param <T>
	 */
	public static<F,T> T entityToModelWithoutId(F entity, Class<T> modelClass) {
		log.debug("entityToModel : Entity属性的值赋值到Model");
		Object model = null;
		if (entity == null || modelClass ==null) {
			return null;
		...
		BeanUtils.copyProperties(entity, model, "id");  // 排除 id
		return (T)model;
	}
