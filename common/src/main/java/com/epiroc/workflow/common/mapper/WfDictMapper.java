...
import com.epiroc.workflow.common.entity.WfDict;
import com.epiroc.workflow.common.system.vo.DictModel;
import com.epiroc.workflow.common.system.vo.DictModelMany;
import com.epiroc.workflow.common.system.vo.DictQuery;
import com.epiroc.workflow.common.system.vo.TreeSelectModel;
 ...
 * @since 2018-12-28
 */
public interface WfDictMapper extends BaseMapper<WfDict> {
	 ...
	 * @param id
	 */
	@Select("delete from wf_dict where id = #{id}")
	 ...
	 * @return
	 */
	@Select("select * from wf_dict where del_flag = 1")
	public List<WfDict> queryDeleteList();
	 ...
	 * @param id
	 */
	@Update("update wf_dict set del_flag = #{flag,jdbcType=INTEGER} where id = #{id,jdbcType=VARCHAR}")
    ...
    List<WfDict> getDictListByLowAppId(@Param("lowAppId") String lowAppId, @Param("tenantId") Integer tenantId);

	/**
	 * 查询被逻辑删除的数据（根据租户id）
	 * @return
	 */
	@Select("select * from wf_dict where del_flag = 1 and tenant_id = #{tenantId}")
	List<WfDict> queryDeleteListBtTenantId(@Param("tenantId") Integer tenantId);
